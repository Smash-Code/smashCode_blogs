...
import { CloudUpload, Clear, PermMedia, CardMembership,Visibility } from "@material-ui/icons";
...
const MediaUpload = ({ data }) => {
        ...
        if (formData.file) {
            return new Promise((resolve, reject) => {
                if (formData.file.name) {
                    let task;
                    var storageRef = storage.ref("uploads/media/" + formData.file.name + id);
    ...
    const PostNewMedia = () => {
        ...
        setProgress(6);
        let key = db.ref('uploads/media').push().key;
        imageUploadToDB(key).then((res) => {
            db.ref('uploads/media').child(key).set({
                url: res,
            ...
            <div className="w-100">
                        ...
                        {formData.file ?
                            <>
                                ...
                                <label htmlFor="imageUpload">
                                    <img ref={imgRef} src={formData.file} alt="" />
                                </label>
                            </>
                            :
                            <>
                ...
                <div className="image-flexbox border-top py-3">
                    <div className="w-100 pb-3 d-flex justify-content-between align-items-center">
                        <Typography variant="h5" >Uploads <PermMedia className="mb-1" /></Typography>
                        {data && data.length > 0 &&
                    ...
                    {data && data.length > 0 ?
                        data.map((item, index) => {
                            return (
                                <div whileHover={{ opacity: 1 }} key={index} className="img-card">
                                    <img src={item?.url} alt="" />
                                    <div className="mt-auto  w-100 mt-auto d-flex justify-content-between align-items-center px-2 py-3">
                                        <div className="">
                                            <span className="text-muted">Uploaded date: <br /> {item?.post_date?.slice(0, 11)}</span>
                                           
                                        </div>
                                        <div className="text-right">
                                        <Button className="" onClick={() => copyURL(item.url)} variant="contained" color="primary">Copy URL</Button><br/>
                                        <Button variant="text" color="inherit" component="a"  href={item?.url} target="_blank" className="mt-1 text-capitalize">view &nbsp;<Visibility/></Button>
                                        </div>
                                    </div>
                                </div>
                            )
