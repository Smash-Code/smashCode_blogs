...
import { CloudUpload, Clear, VideoCall, CardMembership } from "@material-ui/icons";
...
const VideoManage = ({ data }) => {
        ...
        if (formData.file) {
            return new Promise((resolve, reject) => {
                if (formData.file.name) {
                    let task;
                    var storageRef = storage.ref("uploads/videos/" + formData.file.name + id);
    ...
    const PostNewMedia = () => {
        ...
        setProgress(6);
        let key = db.ref('uploads/videos').push().key;
        imageUploadToDB(key).then((res) => {
            db.ref('uploads/videos').child(key).set({
                url: res,
            ...
            <div className="w-100">
                ...
                <div className="media-upload-wrapper position-relative">
                    <div onClick={selectImg} className="selectImg-wrapper img-select-btn-full mt-3 mr-2">
                        {formData.file ?
                            <>
                                <video ref={imgRef} controls></video>
                            </>
                            :
                            <>
                ...
                <div className="image-flexbox border-top py-3">
                    <div className="w-100 pb-3 d-flex justify-content-between align-items-center">
                        <Typography variant="h5" >Uploads <VideoCall className="mb-1" /></Typography>
                        {data && data.length > 0 &&
                        ...
                        data.map((item, index) => {
                            return (
                                <div whileHover={{ opacity: 1 }} key={index} className="img-card">
                                    <video controls src={item?.url} alt="" />
                                    <div className="mt-auto py-3 w-100 mt-auto d-flex justify-content-between align-items-start px-2">
                                        <div className="mr-2">
                                            <span className="text-muted">Uploaded date: <br/> {item?.post_date?.slice(0,11)}</span>
                                        </div>
                                        <Button className="" onClick={() => copyURL(item.url)} variant="contained" color="primary">Copy URL</Button>
                                    </div>
                                </div>
                            )
