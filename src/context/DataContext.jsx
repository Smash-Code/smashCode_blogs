...
const DataContextProvider = ({ children }) => {
    ...
    useEffect(() => {
        db.ref('blogs').limitToLast(24).once('value').then((snap) => {
            if (snap.val()) {
        ...
        db.ref('categories').once('value').then((cate_snap) => {
            if (cate_snap) {
        ...
        db.ref('blogs').once('value').then((blogs_snap) => {
            if (blogs_snap) {
    ...
    useEffect(() => {
        db.ref('blogs').limitToLast(12).once('value').then((snap) => {
            if (snap.val()) {
                let blogsarry = Object.values(snap.val()).map((data) => data)
